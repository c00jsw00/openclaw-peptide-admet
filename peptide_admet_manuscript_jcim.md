...
4. **The PAMPA/MDCK target is left-censored, which caps its R².** The logPapp target (7,283 rows) has only 648 unique values (0.01 quantization); 269 rows (3.7%) are exactly −10.0000, the assay's detection floor (true values ≤ −10, unknown). These censored points carry 49.6% of the total sum of squares. A dedicated ceiling study (reproducible scripts in `analysis/`) shows the censored molecules can be *partially ranked* but not reliably flagged (best LightGBM floor classifier AUC 0.856 on the test split; the MLP's own predictions rank the floor at AUC 0.762, but at the validation-tuned threshold the classifier's precision is only 0.12, and the two-stage method collapses to R² −1.21); the model already reaches R² 0.632 on the uncensored subset, and the theoretical ceiling (uncensored perfect, censored → global mean) is 0.5387. Five improvement routes were tested under the identical leakage-controlled split — rank-Gaussian target transform (0.434 ± 0.011, multi-seed), LightGBM × 128 hyperparameters × 4 feature sets + top-5 ensemble (0.4234), a two-stage floor method, a soft posterior-mean blend (0.4651), and a Tobit censored-likelihood model (0.4056 ± 0.024) — and none exceeded the v4.2 baseline (0.4642) beyond retraining noise. We additionally tested a sixth route: the exact ChemBERTa-77M-MLM embeddings that PeptiVerse (Nat. Commun. 2026) reports as its best SMILES representation for PAMPA (Spearman ρ = 0.69 vs 0.59 for PeptideCLM), under the identical split and training loop (4 feature configurations × 3 seeds). ChemBERTa alone underperformed MoLFormer-XL (ΔR² = −0.009 to −0.020), and concatenating both embeddings gave +0.004 (PAMPA) / −0.003 (Caco-2) — below the seed-to-seed noise band — so the published advantage does not transfer to our canonical-peptide, left-censored data. Raising PAMPA R² toward 0.70 requires uncensored re-measurements of the floored compounds, not a better model. We cross-validated this ceiling against an external dataset: training directly on the PeptiVerse paper's own raw PAMPA (6,869 rows) and Caco-2 (606 rows) permeability data (HuggingFace `ChatterjeeLab/PeptiVerse_data`, using the authors' precomputed ChemBERTa-77M-MLM embeddings plus RDKit/Morgan 2D descriptors, identical training loop, 3 seeds) yields a best test R² of 0.434 (PAMPA; non-floor subset 0.603) and 0.430 (Caco-2) — below 0.70 because that PAMPA dataset carries the same −10 censored floor (240 rows, 3.5%, 49.9% of the total SS; oracle ceiling 0.501 vs our 0.539), so the censored-floor ceiling holds across datasets. We also tested whether the
label-averaging preprocessing of the pepADMET platform (J. Chem. Inf. Model.
2026, 66, 936; arithmetic mean of replicate measurements per molecule,
reported permeability R² 0.435–0.657) could explain higher R² on our data:
it is a no-op there (1.01 measurements per SMILES; ΔR² −0.015 / +0.003 on
PAMPA / Caco-2, within seed noise), so the residual gap is attributed to
that platform's random 8:1:1 splits without SMILES-level train/test
separation. Caco-2 is more severely bounded (repeat within-group SD ≈ 0.97 log units ≈ the target SD; ceiling ≈ 0.14 by the same logic), so its R² 0.3909 is likewise label-quality-limited.
...
9. **Foundation models are the first of the nine routes to beat the v4.2 baseline — and they approach the censored-floor ceiling, which still holds.** As a ninth route we tested two pretrained foundation models under the identical leakage-controlled split: the TabPFN v2 tabular foundation model (no gradient update on our data — its in-context attention *is* the model) and a fine-tuned KPGT graph transformer (the 12-layer LiGhT backbone of KPGT, ICML 2024, with its ICML-pretrained checkpoint and a re-initialized regression head). TabPFN v2 with the 217 RDKit 2D descriptors only (no fingerprints, no pretrained molecular embedding) reaches R² 0.4962 ± 0.0016 (3 seeds), a +0.032 improvement over the v4.2 LightGBM baseline (0.4642) that is far outside the seed noise band — the first of nine routes to beat the baseline on the floor-included test set. The gain is localized to the censored region: non-floor R² is unchanged (0.6268 vs 0.6317). Adding Morgan fingerprints (2265 dim) or MoLFormer-XL embeddings (3033 dim) *degrades* TabPFN (0.4813 / 0.4820), indicating the 217 descriptors are what the model uses. The KPGT fine-tune (per-epoch checkpoint, early-stopping patience 15, 3 seeds) reaches R² 0.5134 ± 0.0048 — the best of all nine routes tested (+0.049 vs baseline), but its non-floor R² is 0.5357, below the baseline's 0.6317: gradient fine-tuning on 3.7%-left-censored labels teaches the floor pattern at a measurable cost to predictions on measured molecules. The practical takeaway: the frozen-embedding negative result (§3.4) is not a general statement that pretrained knowledge cannot help — a *foundation* model (TabPFN, which conditions on the training set at inference time without any weight update) adds a small but real amount of predictive power, and a pretrained graph transformer fine-tuned end-to-end (KPGT) adds more — but the ceiling analysis remains the binding constraint: even the best route tested (KPGT 0.5134) sits below the 0.5387 oracle ceiling (within 0.025 of it), and 0.70 remains unreachable without uncensored re-measurements.
