...
We moved a reproducibility benchmark from the synthetic demo that certified it to **real experimental data**, keeping the protocol identical: a leakage-audited per-modality split (exact-anagram collapse for sequence endpoints, unique-SMILES for molecular endpoints), a shared model definition, and measured-only metrics over four ADMET endpoints. On real data the dual-split comparison becomes decisive — a random half-life split reports R² 0.8733, but the leakage-controlled honest number is 0.6973 — and a frozen ESMC-600M protein-language-model embedding, added without any training of its own, lifts the sequence endpoints to AUC 0.8348 (Hemolysis) and R² 0.6973 (half-life). The released repository now contains the prepared data, all four model weights, scalers, the committed ESMC embedding caches, and `metrics.json`, so every number is re-derivable from the repository. Peptide ADMET numbers should be reported with their split provenance and input modality; we demonstrate the standard on real data.

**Availability**: all code, the four trained models (sequence endpoints 438,529 params, d = 1,580 with ESMC; molecular endpoints 613,889 params, d = 2,265), the scalers, the prepared data, the ESMC embedding caches, and `metrics.json` are at https://github.com/c00jsw00/openclaw-peptide-admet.
...
- **S4.** `train_v41.log` — the human-readable record of the v4.1 training run (per-endpoint training curves, split counts, final SUMMARY block).
- **S5.** `data/esmc/esmc_emb_hemolysis.npz` / `data/esmc/esmc_emb_half_life.npz` — the committed frozen ESMC-600M embedding caches (1,152-dim per sequence; 8,719 and 1,763 rows respectively), so retraining and cached-sequence inference never need the ESMC environment.
